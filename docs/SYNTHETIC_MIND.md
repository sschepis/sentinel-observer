...
## 6. First result of the prediction (2026-09-09)

The prediction in §1 was run (task 41: shuffled ConceptNet corpus, 65 feeds of 1,000 rows, 200 held-out claims with both ends among the taught words). Held-out recovery rose 8.0% → 38.0% with zero wrong answers in 13,000 probes, all of it symbolic inference (is-a chains and inheritance; the graded layer contributed nothing). The network entropy of task 39 fell 10.156 → 9.424 bits per concept. And the correlation between the per-step entropy drop and the per-step recovery gain was **−0.06 per step and +0.39 in five-step blocks — the wrong sign.** The entropy drop shrinks steadily through the run (a concept's own slots fill early; a second edge into a filled slot moves the measure by nothing) while recovery gains hold flat, because recovery grows with the *density of chains* through the graph, which a per-concept slot count cannot see.

So the prediction, as stated over that measure, is false, and the lesson is specific: the measure counted what the observer *holds*, the prediction is about what it can *derive*. The principle itself says as much — coupling matters as mutual observational capability — so the entropy that should fall as the mind forms is over the inferential closure: for each concept, the uncertainty of what the observer could answer about it *through the graph*, not from its own edges alone. Task 39 is to be redefined that way before task 41 is run again; §3 rows A, B and I inherit the change.
