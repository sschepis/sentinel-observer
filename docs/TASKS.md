...
| # | Task | Who | Note |
|---|---|---|---|
| 63 | **Word-problem bench and the honesty fix.** `curriculum/problemsBenchmark.test.ts` (bench config) samples `corpus/problems.jsonl` (SVAMP + ASDiv, 3,004 rows) and reports right / wrong / abstained by source and by what each problem needed. **First run: right 5 · wrong 19 · abstained 276** — the general story parser added or multiplied the first two numbers of the first sentence on comparisons ("how many more … than" → the sum), residuals, rates, divisions asked as a number of groups, part-of-a-whole and unknown-start stories. Fixed in `rules/parse.ts`: the parser now counts quantities over the whole body (exactly two, none in the question, no decimals), declines comparison / share / remainder / unknown-start questions, stated wholes and stated differences in the body, equal groups whose question counts the groups, sums with no join between the numbers, sums asked about one of two kinds, and sums asked as a third kind without a total cue (girls + boys → "pupils" is right, elephants + tigers → "legs" is not; the parser cannot tell them apart without the relation store → 65). **After: right 15 · wrong 0 · abstained 285 on the same 300; right 10 · wrong 0 · abstained 290 on the next 300.** Answered 3–5 %, exact when it answers. The bench asserts wrong ≤ max(2, 10 % of attempted). | Claude — done | `PROBLEMS_SAMPLE`, `PROBLEMS_SKIP`, `PROBLEMS_SHOW`, `PROBLEMS_TAUGHT`; artifacts `bench/curriculum/word-problems-<date>-n<N>[-skip<K>].json` |
| 64 | **A story-state engine** (was #37/#22): entities × quantities, events (gain, loss, transfer, equal groups, sharing), and question types (total, residual, difference, share, start state) compiled to rewrite terms — so subtraction and division problems derive instead of declining, still with a trace. Build for the largest buckets first: 3+ quantities (41 % of problems), decrease/residual (41 %), rate each/per (31 %), then comparison and sharing. Gate: the bench's wrong count stays at 0 while answered climbs. | Claude | the general parser becomes the fallback, not the engine |
...
| 66 | **Vocabulary is the first wall**: most abstentions are "I do not know what *left* / *width* / *apples* means" — the checkable path requires every word known. Let a word problem run when its numbers and cue words parse even if a noun is unknown (the noun is a label, not a fact the answer depends on), and grow the noun as a vocabulary entry from the problem. | Claude | measure on the bench: abstained-for-vocabulary vs abstained-for-shape |
