...
| 77 | **`dialogue.jsonl` is still missing** — the DailyDialog fetch keeps 0 rows (the mirror serves one utterance per row and the grouping key is not among the ones the converter tries). The conversation channel is the one the creative layer unlocks on, so this is the biggest remaining data gap. Needs the fetcher's "first page kept nothing — row columns: …" line from a run with network access. | Sebastian runs the fetch; Claude fixes the shape | `cli/fetch-hf.ts` `dialogueConverter` |
