...
| 60 | **Ingest scaling.** At 26k edges / 26k words (6.3k grown) a relation-graph rebuild costs ≈1.5–2 s warm (≈8 s cold: hologram bind of 38k edges over 17k subjects + the loose re-extraction of 20k definitions + hypothesis refresh), and a 1,000-row feed triggers several rebuilds — 38 s per feed under load, vs the ≈1 s the scale bench measured without vocabulary growth. Make the hologram incremental (re-bind only subjects whose edges changed), cache the loose extraction by a definitions version, and rebuild once per feed. | Claude | `OBSERVER_PROFILE_REBUILD=1` prints the stage timings |
| 61 | **Question latency at corpus scale.** One yes/no question takes ≈1–2 s on that graph (≈150 ms without the corpus). `chain.ts` walks now index the relation array once per rebuild (WeakMap by identity) instead of scanning it per call; the rest is being profiled (`OBSERVER_PROFILE_CHAT=1` prints per-stage laps). Target: back under 200 ms at 70k edges. | Claude | blocks a usable bench at full corpus and the live chat once the corpus lands |
