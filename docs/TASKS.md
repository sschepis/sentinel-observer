...
| 65 | **Ask the store before summing kinds**: "how many pupils" over girls and boys derives only if `pupil` is an is-a ancestor of both in the relation store (ConceptNet gives it); "how many legs" over elephants and tigers needs has-part edges with counts, which the store does not hold → decline. The first place the arithmetic path and the knowledge path meet. | Claude | after 64's first slice |
| 66 | **Vocabulary is the first wall — but not for arithmetic.** Most abstentions read "I do not know what *left* / *width* / *apples* means". Measured after #64: the wall does NOT block the story engine, because the rewrite dispatch runs before the vocabulary ask, so every story the engine reads is answered whatever words it contains. What remains is honesty of the ask itself: a numeral or a number word must never be reported as an unknown referent ("I do not know what \"twenty-three\" means" — `extractUnknownSubject` treats any token over two characters as a possible referent), and for a story the engine declines, asking about an unknown noun is legitimate. | Claude | measure on the bench: abstained-for-vocabulary vs abstained-for-shape |
