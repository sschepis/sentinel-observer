...
## 22. The prototype-shadowing bug class ("constructor")

**The symptom.** Starting the training loop threw `function is not iterable
(cannot read property Symbol(Symbol.iterator))`, and after the first fix, one
layer down: `Non-finite value for "clampRange value": NaN`.

**The cause — twice, one class.** The deck contains the word "constructor".
`semanticAssignment` built its vocabulary as a plain `{}`; the guard
`if (vocabulary[word] !== undefined)` read the INHERITED
`Object.prototype.constructor` function, so the word never received a prime
signature, and every later read of `vocabulary['constructor']` returned that
function: the curriculum's `new Set(signature)` threw the iterator error.
`drillFailuresSnapshot()` had the same shape: `Object.fromEntries` over a
plain object, so `ctx.drillFailures?.['constructor']` returned the same
function and `function / 3 → NaN` threw inside `clampRange`.

**The fixes.** (1) `semanticAssignment` and `deckVocabulary` now build their
word-indexed records on a null prototype, so a deck word named after any
`Object.prototype` member reads as its own entry — or undefined — never as
the inherited function. Measured against the pre-fix table: **0 changed
signatures across all 20,273 words; 1 added** ('constructor' → [409, 1597,
569, 1123]). (2) `neighborhoodEdges` treats a non-array vocabulary entry as
no-evidence (0 edges) instead of iterating it. (3) `drillFailuresSnapshot`
returns a null-prototype record. All other word-keyed state (composition
weights, exposure/encounter counts, cue confidence) was audited: they are
`Map`/`Set`s, immune by construction.

**The record.** The shipped `public/bootstrap.json` was trained under the old
table and carries its fingerprint (c1b5f183), so the app's loud
vocabulary-fingerprint check would reject it. Regenerated by importing the
record, teaching 'constructor' under the new table, and re-stamping:
**c1b5f183 → e825aa55, 1479 traces, 751 words** — old and new records answer
identically (cold 'hello' → ask; after priming → memorized 0.638 on both).

**The gate.** `PrototypeShadowing.test.ts` (5 tests): every one of the
20,250 deck words carries an own array signature (none reads back as a
function), every word scores through the real curriculum without throwing
and with finite components, `teach('constructor')` stores, and the
drill-failures snapshot never surfaces the inherited function.

**Controls:** web **823/823** (818 + 5 new), core **271/271**, typecheck
clean, margin bench unchanged (+0.129 / top-1 99.0% / DC 0.732 / competency
